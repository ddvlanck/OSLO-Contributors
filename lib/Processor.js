...
class Processor {
    ...
    async processJSONFile(object) {
        if (object.report && object.report != "null") {
            await this.fetchFile(object.report)
                .then(data => {
                    this.processData(data, object.name, object.status);    //TODO: change to object.name for Toolchain
                })
                .catch(error => {
                    console.log(error);
    ...
    processData(data, name, status) {
        ...
        contributorsAndAffiliations.set(name, {contributors: anonData, published: data['publication-date'], status: status});
    }
