...
class Processor {
    ...
    async processJSONFile(object) {
        if (object.report && object.report != "null") {
            await this.fetchFile(object.report)
                .then(data => {
                    this.processData(data, object.name, object.status, object.publicationDate);
                })
                .catch(error => {
                    console.log(error);
    ...
    processData(data, name, status, publicationDate) {
        ...
        const anonData = this.transformData(data['contributors'].concat(data['authors'], data['editors']));
